...
| 計時器（5400 tick ≈ 10.8 分鐘）| 超時後比佔點數，平手比主堡剩餘 HP |
...
**目標：** 讓每場戰鬥可以選不同地圖，避免永遠同一張。

**做什麼：**
- 建立 `createMap(template: MapTemplate, seed?: number): { cells, zones, lanes }` 函式
- 實作 4 種預設模板：

| id | 名稱 | 特性 |
|---|---|---|
| `standard` | 標準戰場 | 三路、稀疏森林、高地 |
| `fortress`  | 要塞戰場 | 三路、每路兩佔點、有城門阻斷 |
| `jungle`    | 密林戰場 | 三路、大範圍森林（視野壓縮） |
| `blitz`     | 閃擊戰場 | 三路、無地形阻礙、快速對決 |

**涉及檔案：** `engine/mapData.ts`、`stores/gameStore.ts`（傳入模板參數）
