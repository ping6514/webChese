...
| **SP 技能觸發** | `triggerSPSkill()` 實作 5 種隊長技能，SP 滿自動觸發（auto 模式） |
| 戰吼衝鋒 | Buff 型：下一擊 ×1.6 + 清目標 ATB（charge_roar，步兵隊長） |
| 鐵甲壁壘 | 時限型：60 tick 防禦 ×2、移速歸零（iron_wall，重甲隊長） |
| 鐵騎踐踏 | 即時型：警戒範圍所有敵方 ATK×0.8 + ATB 清零（trample，騎士隊長） |
| 箭雨 | 即時型：主目標周圍 1 格所有敵方 ATK×1.5（arrow_rain，弓手隊長） |
| 破城突擊 | 即時型：推進最近未佔領區/主堡 captureHp 20%（breach，攻城隊長） |
| SP 技能 FX | `sp_skill` 事件，renderer 顯示金色技能名浮字（1.6s）＋ flash |
