...
## Web 系統實作對齊（目前版本）
- 回合開始：儲存池中每 1 點魔力 → 轉換為 **2 點財力**（`storageToGoldRate = 2`），之後儲存池清空。
- 購買階段：
  - 展示區靈魂卡：2 財力。
  - 隨機抽取：1 財力。
  - 購買敵方墳場最上方靈魂卡：**3 財力**。
  - 購買次數：購買靈魂卡 / 購買道具卡，各自每回合 **1 次**。
- 死靈術階段：系統目前每回合基礎 **1 次**死靈術行動（附魔/復活）。
- 血之祭儀：**已實作**。每回合限 1 次，帥 -3 HP，本回合死靈術行動次數 +1。
- 復活：花費 **3 財力**。
- 戰鬥階段：移動 1 魔力 / 射擊 1 魔力。

### 戰鬥：射擊 UI 實作備註（Web）

- 系統採用「preview-first」流程：點選射擊目標後 **不會立刻射擊**，需要明確 `Shoot (Enter)` 才會執行。
- 目標附近會出現可拖拉的射擊選單：`Shoot / Cancel / Shoot Preview`。
- `Shoot Preview` 會開啟詳細彈窗（傷害與 effects 列表）；彈窗關閉後選單會回到上次拖曳位置。
- 多目標效果在棋盤上以中文標記提示：
  - `貫N`：貫通連帶命中順序
  - `濺`：波及連帶命中
  - `連?` / `連`：連鎖可選 / 已選第二目標
...
- 獻祭使用棋盤上的浮動選單進入目標選擇（不放在 SidePanel）。
- 確認彈窗（ConfirmModal）出現時，獻祭選單會自動隱藏以避免遮擋。

### 戰鬥：技能啟動 FX（Web）

- 當能力實際生效時，引擎會發出 `ABILITY_TRIGGERED` 事件。
- UI 會在棋盤上顯示浮字 + 紫藍高亮，協助玩家理解「剛剛是什麼能力啟動」。

### Web：資訊面板溢出處理

- Graveyard 顯示採折疊/下拉清單避免長 id 造成溢出。

（註：進階規則若與系統不同，以系統 UI 與引擎實作為準。）
...
3. **死靈術階段**：執行附魔、復活（系統目前基礎 1 次；可使用血之祭儀增加次數）。
...
flowchart TD
    ...
    subgraph NecroPhase [死靈術階段: 系統目前基礎 1 次行動]
...
* **盜取靈魂卡 (3 財力)**：購買敵方墳場「最上方」第一張靈魂卡（順序不可變更）。
...
本階段為強化陣容的核心，系統目前每回合基礎可執行 **1 次** 行動。
...
- **行動 B：復活**：
    ...
    3. **復活贈送（桌遊規則）**：復活後可立即執行 1 次**免費附魔**（不消耗死靈術行動次數，但仍需支付靈魂卡成本）。

（註：Web 系統目前未實作「復活贈送免費附魔」。）
- **血之祭儀**：每回合限 1 次。帥失去 3 點 HP（不可減免），本回合死靈術行動次數 +1。

（註：Web 系統已實作血之祭儀，並提供確認彈窗提示 HP 變化。）

### 補充規則：
* **強制剝離**：若欲更換活單位的靈魂卡，必須透過「單位戰死」、「主動獻祭」或「特定道具效果（如：亡者歸途）」將靈魂移除後，方可重新附魔。
* **道具增益**：若道具提供『額外死靈術動作』，不計入每回合 1 次的基礎限制。
...
| **馬** | 舞影 | 5 | 物理 2 | 1/0 | **9** | 執行「移動」時忽略所有路徑阻擋（永遠不會「拐馬腳」）。 |
| **砲** | 夜華 | 6 | 物理 2 | 0/1 | **9** | **【越境】**：射擊產生「濺射」，目標格相鄰 1 格的敵方受到同等傷害（本回合限 1 次）。 |
| **砲** | 風華 | 5 | 物理 2 | 0/1 | **9** | **【越境】**：射擊傷害 +2。 |
...
| 基底 | 名稱 | 召喚成本(財力) | ATK | DEF (物/魔) | **總 HP** | 完整效果描述 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| **車** | 骨華 | 6 | 物理 3 | 1/1 | **11** | **【屍氣】**：≥ 6 時獲得 **【貫穿】** (前 2 個敵方)；≥ 7 時，射擊傷害額外 +1。 |
| **車** | 冥骨車 | 7 | 物理 3 | 1/0 | **12** | **【獻祭】**：射擊前摧毀 1 格內友軍，本次射擊獲得 **【無視阻擋】** 且傷害根據目前屍骸數 +1 (上限 +2)。 |
...
| **士** | 魂侍 | 3 | 魔法 2 | 1/2 | **8** | **【永夜血脈(魔法)】**：屍氣 ≥ 3 時全場永夜單位魔防 +1。**【獻祭】**：摧毀自身使「帥」無敵 1 回合。 |
...
### v1.23 → v1.24（模擬驗證調整）
> 依 N=500 場自對局模擬結果（winRate 信賴區間 95% Wilson）

| 卡牌 | 氏族 | 調整方向 | 調整內容 | 調整前 winRate | 理由 |
|:---|:---|:---:|:---|:---:|:---|
| **骨華**（車） | 永夜骸骨 | 🔽 削弱 | 貫穿觸發屍氣門檻 3→5→**6**；傷害加成 +2→+1，門檻 6→7；HP 12→**11** | 0.704 → 0.652 | 二次調整，仍超 0.57 閾值 |
| **冥骨車**（車） | 永夜骸骨 | 🔽 削弱 | 獻祭傷害加成上限 +4→+2 | 0.571 | 略強，犧牲後輸出過高 |
| **魂侍**（士） | 永夜骸骨 | 🔼 增強 | 召喚成本 4→3 | 0.470 | 略弱且使用率偏低，降低入場門檻 |
| **風華**（砲） | 暗月影刃 | 🔼 增強 | 越境傷害加成 +1→+2 | 0.447 | 略弱，效果相比同成本卡不足 |
